# 动态配置修改iss文件
# 修改 MyAppName "aa"
# 修改 MyAppVersion "1.0.0"
# 修改 MyAppPublisher "121414"
# 修改 MyAppURL "https://www.1414.com/"
# 修改 MyAppExeName "aa.exe"
# 修改 MyAppAssocName MyAppName + "文件"
# 修改 MyAppAssocExt ".myp"
# 修改[Files]配置(指导链接：https://www.cnblogs.com/pengsn/p/13343267.html)，修改主执行文件路径和关联文件夹路径为部署文件夹对应路径，如下例如
# '''
# [Files]
# Source: "部署文件夹\{#MyAppExeName}"; DestDir: "{app}"; Flags: ignoreversion
# Source: "部署文件夹\exeOther\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
# '''
# 修改OutputDir为安装包输出文件路径
# 修改SetupIconFile为自定义路径


#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
自动化混淆打包工具（原地混淆 + 文件整理 + 动态 ISS 生成）
...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QGroupBox, QFormLayout, QTabWidget
)
...
class WorkThread(QThread):
    ...
    def run(self):
        try:
            ...
            # 2. 整理部署文件夹
            self.log("[2/4] 整理部署文件夹...")
            if not self._prepare_deploy_folder():
                self.finished_signal.emit(False, "整理部署文件夹失败")
                return

            # 3. 生成 ISS 脚本
            self.log("[3/4] 生成 Inno Setup 脚本...")
            iss_path = self._generate_iss()
            if not iss_path:
                self.finished_signal.emit(False, "生成 ISS 脚本失败")
                return
            ...
            self.finished_signal.emit(True, f"打包完成！安装包位于: {self.config['output_dir']}")
    ...
    def _prepare_deploy_folder(self):
        """将混淆后的 EXE 复制到部署根目录，其他文件复制到 exeOther"""
        try:
            ...
            self.log(f"整理部署文件夹异常: {str(e)}")
            return False

    # ---------- 生成 ISS 脚本 ----------
    def _generate_iss(self):
        """读取模板，替换所有占位符，生成临时 ISS 文件"""
        try:
            ...
            replacements = {
                ...
                '{#MyAppIconPath}': self.config['icon_path'],
                '{#OutputDir}': self.config['output_dir'],
                '{#DeployDir}': self.config['deploy_dir'],
            }
            for key, value in replacements.items():
                content = content.replace(key, value)

            # 动态生成 [Files] 区段（使用 DeployDir 和 AppExeName）
            # 注意：需要确保模板中有一个专门的占位符 {#FILES_SECTION} 用于替换
            # 如果模板中没有该占位符，也可以直接替换整个 [Files] 区段，但为了灵活性，我们使用占位符
            files_section = self._generate_files_section()
            ...
            with open(temp_iss, 'w', encoding='utf-8') as f:
                f.write(content)
            self.log(f"  已生成临时 ISS 文件: {temp_iss}")
            return temp_iss
        except Exception as e:
            self.log(f"生成 ISS 脚本异常: {str(e)}")
            return None
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("自动化混淆打包工具（全面动态配置）")
    ...
    def _init_ui(self):
        ...
        # 工具路径
        self.reactor_edit = self._create_browse_row(" .NET Reactor:", self.default_reactor, "exe")
        self.iscc_edit = self._create_browse_row(" Inno Setup 编译器:", self.default_iscc, "exe")
        ...
        # 路径配置
        self.input_exe_edit = self._create_browse_row("待混淆 EXE (原地覆盖):", "", "exe")
        self.deploy_dir_edit = self._create_browse_row("部署文件夹 (整理后文件):", "", "dir")
    ...
    def _create_browse_row(self, label_text, default_text, file_type):
        """辅助方法：创建带浏览按钮的行，返回 (label, widget_layout) 或直接返回 (label, widget) 供 QFormLayout 使用"""
        ...
        # 返回一个元组 (QLabel, QHBoxLayout) 以便在 addRow 中使用
    ...
    def start_process(self):
        ...
        # 校验必填项
        required = ['reactor_path', 'iscc_path', 'nrproj_template', 'iss_template',
                    'input_exe', 'deploy_dir', 'output_dir', 'app_exe_name']
        for key in required:
            if not config[key]:
