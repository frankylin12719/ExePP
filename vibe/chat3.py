# 将经过混淆的exe文件，复制到选定文件夹，
# 并将输入文件夹中其他文件归纳选定文件夹的exeOther文件夹内，
# 并对应调整.iss文件的[Files]为选定文件夹

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
自动化EXE混淆+打包工具（支持文件整理与动态生成[Files]节）
...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QGroupBox, QFormLayout, QCheckBox
)
...
class WorkThread(QThread):
    ...
    def __init__(self, reactor_path, iscc_path, nrproj_template, iss_template,
        ...
        self.deploy_dir = deploy_dir   # 用户指定的部署文件夹
    ...
    def run(self):
        try:
            ...
            # 2. 修改 .nrproj 并执行混淆
            self.log("[1/5] 准备 .NET Reactor 项目文件...")
            nrproj_modified = self._prepare_nrproj(self.nrproj_template,
                                                   self.input_exe,
                                                   obfuscated_dir)
            self.log("[2/5] 执行 .NET Reactor 混淆...")
            ...
            # 4. 修改 .iss 脚本（动态生成[Files]节）
            self.log("[4/5] 生成 Inno Setup 脚本...")
            iss_modified = self._prepare_iss(self.iss_template,
                                             self.deploy_dir,
                                             self.output_dir)
            # 5. 执行 Inno Setup 打包
            self.log("[5/5] 执行 Inno Setup 打包...")
    ...
    def _prepare_deploy_folder(self, obfuscated_exe, deploy_dir):
        """创建部署文件夹，复制混淆exe，并将输入文件夹中其他文件复制到 exeOther"""
        try:
            # 清空并重建部署文件夹
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("自动化混淆打包工具（支持文件整理）")
    ...
    def _init_ui(self):
        ...
        io_layout.addRow("输出目录（安装包位置）:", output_dir_layout)
