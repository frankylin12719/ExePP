# 混淆是在输入文件夹原地混淆，混淆的exe文件会覆盖原来的exe,
# 那么不需要指定输出目录给.NET Reactor（输出目录为输入文件夹本身），
# 可以直接从输入文件夹复制混淆后的exe到部署文件夹根目录，
# 其他文件到exeOther，进行打包

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
自动化EXE混淆+打包工具（原地混淆，直接整理输入文件夹）
...
class WorkThread(QThread):
    ...
    def __init__(self, reactor_path, iscc_path, nrproj_template, iss_template,
                 input_exe, output_dir, deploy_dir, parent=None):
        ...
        self.input_exe = input_exe          # 待混淆的 exe 路径（也是输入文件夹中的主 exe）
        self.output_dir = output_dir        # 最终安装包输出目录
        self.deploy_dir = deploy_dir        # 部署文件夹（用于整理文件）
    ...
    def run(self):
        try:
            input_dir = os.path.dirname(self.input_exe)   # 输入文件夹
            main_exe_name = os.path.basename(self.input_exe)

            # 1. 修改 .nrproj 并执行混淆（输出目录直接设为输入文件夹）
            self.log("[1/4] 准备 .NET Reactor 项目文件（输出目录=输入文件夹）...")
            nrproj_modified = self._prepare_nrproj(self.nrproj_template,
                                                   self.input_exe,
                                                   input_dir)   # 关键：输出目录为输入文件夹
            self.log("[2/4] 执行 .NET Reactor 混淆（原地覆盖）...")
            success = self._run_reactor(self.reactor_path, nrproj_modified)
            if not success:
                self.finished_signal.emit(False, "混淆失败，请查看日志")
                return

            # 混淆后，输入文件夹中的主 exe 已被覆盖（或替换）
            # 检查文件是否存在
            if not os.path.isfile(self.input_exe):
                self.finished_signal.emit(False, f"混淆后未找到主 exe: {self.input_exe}")
                return
            self.log(f"混淆完成，主程序已更新: {self.input_exe}")

            # 2. 整理部署文件夹：从输入文件夹复制文件
            self.log("[3/4] 整理部署文件夹...")
            if not self._prepare_deploy_folder(input_dir, main_exe_name, self.deploy_dir):
                self.finished_signal.emit(False, "整理部署文件夹失败")
                return
            self.log(f"部署文件夹已准备: {self.deploy_dir}")

            # 3. 生成 .iss 脚本（动态生成 [Files] 节）
    ...
    def _prepare_nrproj(self, template_path, input_exe, output_dir):
        """修改 .nrproj 中的 InputAssembly、Main_Assembly 和 OutputDirectory"""
    ...
    # ---------- 文件整理 ----------
    def _prepare_deploy_folder(self, input_dir, main_exe_name, deploy_dir):
        """从输入文件夹复制：主exe -> 部署根目录；其他所有 -> exeOther"""
        try:
            # 清空并重建部署文件夹
            ...
            # 1. 复制主 exe 到部署根目录
            src_exe = os.path.join(input_dir, main_exe_name)
            if not os.path.isfile(src_exe):
                raise Exception(f"输入文件夹中找不到主 exe: {main_exe_name}")
            dest_exe = os.path.join(deploy_dir, main_exe_name)
            shutil.copy2(src_exe, dest_exe)
            self.log(f"  已复制主程序: {dest_exe}")

            # 2. 创建 exeOther 目录，复制其他所有内容
    ...
    def _prepare_iss(self, template_path, deploy_dir, output_dir):
        """读取 .iss 模板，替换 {FILES_SECTION} 占位符"""
        with open(template_path, 'r', encoding='utf-8') as f:
            content = f.read()

        # 获取主 exe 名称（部署根目录下的第一个 exe）
    ...
    def _generate_files_section(self, deploy_dir, app_name):
        ...
        lines.append(f'Source: "{deploy_dir_norm}\\exeOther\\*"; DestDir: "{{app}}\\exeOther"; Flags: recursesubdirs ignoreversion')
        return "\n".join(lines)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("自动化混淆打包工具（原地混淆版）")
    ...
    def _init_ui(self):
        ...
        io_layout.addRow("待混淆的 EXE（所在文件夹即输入文件夹）:", input_exe_layout)
    ...
    def start_process(self):
        ...
        if not all([reactor, iscc, nrproj, iss, input_exe, output_dir, deploy_dir]):
        ...
        self.work_thread = WorkThread(reactor, iscc, nrproj, iss, input_exe, output_dir, deploy_dir)
