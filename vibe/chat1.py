# 现有工具dotnet reactor7.0.0,inno setup6.7.1，
# 以及对应的dotnet reactor配置混淆加壳的.nrproj文件和inno setup配置打包的.iss文件（.iss文件参照https://github.com/Mq-b/CppLab/blob/main/md/%E4%BD%BF%E7%94%A8InnoSetup%E5%88%B6%E4%BD%9C%E5%AE%89%E8%A3%85%E5%8C%85.md）

# 已有待混淆打包exe文件夹，
# 请给我生成一个桌面端可用modern-style的自动化配置EXE混淆加壳打包工具(用python编写)
...
class WorkThread(QThread):
    ...
    def __init__(self, reactor_path, iscc_path, nrproj_template, iss_template,
                 input_exe, output_dir, parent=None):
    ...
    def _prepare_nrproj(self, template_path, input_exe, output_dir):
        """读取模板 .nrproj，修改输入和输出路径，返回临时文件路径"""
        tree = ET.parse(template_path)
        root = tree.getroot()

        # 命名空间处理（.nrproj 可能没有命名空间，直接查找标签）
        # 查找 InputAssembly 和 OutputDirectory 元素
        input_asm = root.find(".//InputAssembly")
        if input_asm is not None:
            input_asm.text = input_exe
            self.log(f"  设置输入: {input_exe}")
        else:
            self.log("  警告: 未找到 <InputAssembly> 标签，请检查 .nrproj 模板")

        out_dir = root.find(".//OutputDirectory")
        if out_dir is not None:
            out_dir.text = output_dir
            self.log(f"  设置输出目录: {output_dir}")
        else:
            self.log("  警告: 未找到 <OutputDirectory> 标签，请检查 .nrproj 模板")
